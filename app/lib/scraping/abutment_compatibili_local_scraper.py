...
ECOMMERCE_ID = "14a3a1b0-62ea-436a-88b5-e1de94219301"
SITE_ORIGIN = "https://abutmentcompatibili.com"
...
ROUTES: dict[str, dict[str, object]] = {
    "prodotto": {
        "label": "PRODOTTO",
        "urls": [f"{SITE_ORIGIN}/prodotto.html"],
    },
    "monconi": {
        "label": "MONCONI",
        "urls": [f"{SITE_ORIGIN}/monconi-abutment-compatibili.html"],
    },
    "rhein83": {
        "label": "RHEIN83",
        "urls": [f"{SITE_ORIGIN}/rhein83-2.html"],
    },
    "flusso_digitale": {
        "label": "FLUSSO DIGITALE",
        "urls": [f"{SITE_ORIGIN}/flusso-digitale.html"],
    },
}
...
def parse_product_card(card, page_number: int) -> dict | None:
    ...
    name_link = card.select_one("a.product-item-link")
    product_name = name_link.get_text(" ", strip=True) if name_link else None
    product_url = name_link.get("href", "").strip() if name_link else None
    ...
    *,
    ...
    if session_id is None:
        session_id = prompt_session_id(
            supabase, ECOMMERCE_ID, f"Abutment Compatibili {label}"
        )
    ...
    for url_index, base_url in enumerate(urls):
        ...
        section = label if len(urls) == 1 else f"{label}/{base_url.rstrip('/').split('/')[-1]}"
...
def run_from_config(config: dict) -> None:
    ...
    if not session_id:
        raise ValueError("session_id mancante nella config Abutment Compatibili")
    ...
    for index, route_key in enumerate(routes, start=1):
        if route_key not in ROUTES:
            raise ValueError(f"rotta Abutment Compatibili sconosciuta: {route_key!r}")
    ...
    log("=== Scraping Abutment Compatibili completato ===")


def main(argv: list[str] | None = None) -> None:
    config_path = parse_config_path(argv)

    log("=== Avvio abutment_compatibili_local_scraper ===")
    ...
    require_interactive_tty("python app/lib/scraping/abutment_compatibili_local_scraper.py")
    ...
    if mode == "direct":
        ...
        session_id = prompt_session_id(supabase, ECOMMERCE_ID, "Abutment Compatibili")
        start_page = 1
        log("Modalità diretta: partenza da pagina 1")
    else:
        print()
        print("Quali rotte Abutment Compatibili vuoi eseguire?")
        run_prodotto = prompt_yes_no("Eseguire la rotta PRODOTTO (~260)?")
        run_monconi = prompt_yes_no("Eseguire la rotta MONCONI (~1373)?")
        run_rhein83 = prompt_yes_no("Eseguire la rotta RHEIN83 (~229)?")
        run_flusso = prompt_yes_no("Eseguire la rotta FLUSSO DIGITALE (~7)?")

        selected_routes = []
        if run_prodotto:
            selected_routes.append("prodotto")
        if run_monconi:
            selected_routes.append("monconi")
        if run_rhein83:
            selected_routes.append("rhein83")
        if run_flusso:
            selected_routes.append("flusso_digitale")
